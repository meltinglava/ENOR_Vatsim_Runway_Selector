...
use nom::{
    bytes::complete::{tag, take_till}, character::complete::{char, space0}, combinator::{opt, rest}, sequence::preceded, AsChar, Finish, IResult, Parser
};

use crate::{
    obscuration::{nom_obscuration, Obscuration}, pressure::{nom_pressure, Pressure}, sea_surface_indicator::{self, nom_sea_surface_indicator, SeaSurfaceIndicator}, temprature::{nom_temprature_info, TempratureInfo}, units::timestamp::{nom_metar_timestamp, Timestamp}, wind::{nom_wind, Wind}
};
...
pub fn nom_parse_metar(input: &str) -> IResult<&str, Metar> {
    let (rest, (icao, timestamp, auto, wind, obscuration, temprature, pressure, sea_surface_indicator, nosig, remark)) = (
        ...
        opt(preceded((space0, tag("RMK ")), rest.map_res(|s| match s {
            "" => Err("Empty remark"),
            _ => Ok(s),
        }))),
    )
...
mod tests {
    ...
    fn test_parse() {
        ...
        input.lines().for_each(|line| {
            match nom_parse_metar(line.trim()) {
                Ok((rest, metar)) => {
                    assert!(rest.is_empty(), "Unparsed input remains: '{}'\n input: '{}'\n data: {:?}", rest, line, metar);
                }
                Err(e) => {
