...
pub struct TempratureInfo {
    pub temp: OptionalData<i32, 2>,
    pub dew_point: OptionalData<i32, 2>,
}

pub(crate) fn nom_temprature_info(input: &str) -> IResult<&str, TempratureInfo> {
    separated_pair(
        OptionalData::optional_field(nom_maybe_negative_temp),
        char('/'),
        OptionalData::optional_field(nom_maybe_negative_temp),
    )
    .map(|(temp, dew_point)| TempratureInfo { temp, dew_point })
    .parse(input)
}
...
mod tests {
    ...
    fn test_name() {
        let input = "34/12";
        let expected = TempratureInfo {
            temp: OptionalData::Data(34),
            dew_point: OptionalData::Data(12),
        };
        let result = nom_temprature_info(input);
        assert_eq!(Ok(("", expected)), result);

        let input_with_negative = "M12/M34";
        let expected_with_negative = TempratureInfo {
            temp: OptionalData::Data(-12),
            dew_point: OptionalData::Data(-34),
        };
        let result_with_negative = nom_temprature_info(input_with_negative);
