use nom::{
    bytes::complete::take, character::complete::{self, u32}, combinator::{all_consuming, map_parser, opt, value}, IResult, Parser
};
...
pub(crate) fn nom_pressure(input: &str) -> IResult<&str, Pressure> {
    ...
    (
        opt(hectopascals),
        opt(inches_of_mercury),
    )
        //ensure that at least one of qnh or altimeter is present
        .map_res(|(qnh, altimeter)| if qnh.is_some() || altimeter.is_some() {
...
fn nom_pressure_unit<'a>(pressure_unit: PressureUnit) -> impl Parser<&'a str, Output = PressureUnit, Error = nom::error::Error<&'a str>> {
    value(pressure_unit, complete::char(pressure_unit.pressure_letter()))
}
