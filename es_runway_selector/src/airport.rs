...
pub struct Airport {
    pub icao: String,
    pub metar: Option<metar_decoder::metar::Metar>,
    pub runways: Vec<Runway>,
    pub runways_in_use: IndexMap<RunwayInUseSource, IndexMap<String, RunwayUse>>,
    /// Tags returned by the plugin for the winning selection source.
    pub selection_tags: IndexMap<RunwayInUseSource, Vec<SelectionTag>>,
}
...
impl Airport {
    ...
    /// Select the best runway from the first runway group using METAR wind.
    ///
    /// Returns `Err(NoRunwayToSet)` when:
    /// - No METAR is available.
    /// - Wind difference between runway directions is ≤ 2 kt.
    /// - The airport has multiple runway groups (a plugin should handle those).
    pub fn set_runway_based_on_metar_wind(
        &self,
    ) -> Result<IndexMap<String, RunwayUse>, ApplicationError> {
        if self.runways.len() == 1 {
            self.internal_set_runway_based_on_metar_wind(0)
                .ok_or(ApplicationError::NoRunwayToSet)
        } else {
            // Multi-group airports require a plugin; we have no generic policy.
            Err(ApplicationError::NoRunwayToSet)
        }
    }

    pub(crate) fn internal_set_runway_based_on_metar_wind(
        &self,
        runway_index: usize,
