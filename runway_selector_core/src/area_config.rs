...
//!
//! [`load_area_config`] and [`load_profile_config`] apply the merge and parse
//! the resulting [`AreaConfig`] / [`ProfileConfig`].
...
pub struct AreaManifest {
    ...
    /// subprocess that speaks the gRPC protocol on stdin/stdout (or a Unix
    /// socket — TBD in Phase 4).
    pub entry: String,
    #[serde(default)]
    pub supported_icaos: Vec<String>,
    /// Minimum `runway_selector_core` semver required.
    #[serde(default)]
...
pub fn load_area_config(area_dir: &Path) -> CoreResult<AreaConfig> {
...
pub fn load_profile_config(profile_path: &Path) -> CoreResult<ProfileConfig> {
...
pub fn load_area_manifest(area_dir: &Path) -> CoreResult<AreaManifest> {
    let path = area_dir.join("manifest.toml");
    let raw = fs::read_to_string(&path)?;
    toml::from_str(&raw).map_err(|e| CoreError::AreaConfig(format!("{}: {e}", path.display())))
}

/// Read `<base_path>` and overlay `<base_path with .local.toml suffix>` on it.
/// Generic across the area/profile/top-level config types.
pub fn load_with_local_override<T>(base_path: &Path) -> CoreResult<T>
where
    T: serde::de::DeserializeOwned + Default,
{
    let mut value = if base_path.exists() {
        ...
        toml::from_str::<toml::Value>(&raw)
            .map_err(|e| CoreError::AreaConfig(format!("{}: {e}", base_path.display())))?
    } else {
    ...
    if local_path.exists() {
        ...
        let overrides = toml::from_str::<toml::Value>(&raw)
            .map_err(|e| CoreError::AreaConfig(format!("{}: {e}", local_path.display())))?;
        merge_local_overrides(&mut value, overrides);
    }

    value
        .try_into()
        .map_err(|e: toml::de::Error| CoreError::AreaConfig(e.to_string()))
}
